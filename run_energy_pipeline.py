...
from src.features.energy_feature_engineering import EnergyFeaturePipeline, EnergyFeatureConfig
from src.training.energy_model_training import EnergyModelTrainer, EnergyModelConfig, EnergyEnsembleTrainer
...
class EnergyMLOpsPipeline:
    ...
    def run_feature_engineering(self) -> dict:
        ...
        try:
            ...
            feature_config = EnergyFeatureConfig(
                ...
                create_cyclical_features=self.config["features"]["create_cyclical_features"],
                ...
                feature_selection_method=self.config["features"]["feature_selection_method"],
            ...
            logger.info(f"   🔧 Test features: {X_test.shape if X_test is not None else 'None'}")
        ...
        try:
            ...
            if self.config["training"]["model_type"] != "ensemble":
                ...
                if self.config["deployment"]["register_model"]:
                    ...
                    training_results["single_model"]["registered_version"] = model_version
                ...
                logger.info(f"   📈 Val RMSE: {result['validation_metrics'].get('rmse', 'N/A')}")
            ...
            # Ensemble training
                ...
                for model_type in self.config["training"]["ensemble_models"]:
                    config = EnergyModelConfig(
                        ...
                        cv_folds=self.config["training"]["cv_folds"],
                        n_trials=max(50, self.config["training"]["n_trials"] // len(self.config["training"]["ensemble_models"])),
                        target_transform=self.config["training"]["target_transform"],
                        use_time_series_cv=self.config["training"]["use_time_series_cv"]
                    )
                ...
                ensemble_result = ensemble_trainer.train_ensemble(X_train_split, y_train_split, X_val, y_val)
                ...
                logger.info(f"   🤖 Models: {[config.model_type for config in ensemble_configs]}")
                logger.info(f"   ⚖️ Weights: {ensemble_result['weights']}")
                logger.info(f"   📊 Ensemble RMSE: {ensemble_result['ensemble_metrics']['rmse']:.4f}")
    ...
    def run_model_evaluation(self) -> dict:
        ...
        try:
            ...
            predictions_df = pd.DataFrame({
                ...
                'timestamp': pd.date_range(start='2023-01-01', periods=len(test_predictions), freq='H')
            })
            predictions_df.to_csv("data/processed/test_predictions_energy.csv", index=False)
    ...
    def generate_report(self) -> dict:
        ...
        logger.info(f"   ✅ Data Ingestion: {self.results.get('data_ingestion', {}).get('total_rows', 'N/A')} records")
        logger.info(f"   ✅ Feature Engineering: {self.results.get('feature_engineering', {}).get('train_shape', 'N/A')} features")
        logger.info(f"   ✅ Model Training: {list(self.results.get('model_training', {}).keys())}")
        logger.info(f"   ✅ Model Evaluation: {self.results.get('model_evaluation', {}).get('predictions_saved', 'N/A')}")
...
def main():
    ...
    parser.add_argument("--step", type=str, choices=["data", "features", "training", "evaluation", "all"], 
