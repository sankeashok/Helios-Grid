...
def create_optimized_thumbnail():
    ...
    try:
        ...
        if subtitle_font:
            draw.text((250, 220), subtitle_text, fill=subtitle_color, font=subtitle_font)
        else:
        ...
        badges = ["Azure ML", "FastAPI", "XGBoost", "MLflow", "Docker", "Prometheus", "Grafana"]
        badge_x = 80
        badge_y = 450
        badge_spacing = 90
        ...
        for i, badge_text in enumerate(badges):
            ...
            draw.rectangle([
                x_pos, badge_y,
                x_pos + badge_width, badge_y + 30
            ], fill=(59, 130, 246, 50), outline=(59, 130, 246, 100))
            
            # Badge text
            if badge_font:
                draw.text((x_pos + 10, badge_y + 8), badge_text, fill=(96, 165, 250), font=badge_font)
            else:
        ...
        draw.rectangle([
            github_x, github_y,
            github_x + 280, github_y + 40
        ], fill=(255, 255, 255, 25), outline=(255, 255, 255, 50))
        
        # GitHub text
        if small_font:
            draw.text((github_x + 20, github_y + 12), github_text, fill=(255, 255, 255), font=small_font)
        else:
