...
class HeliosGridDagsHubIntegration:
    ...
    def setup_dagshub(self):
        ...
        try:
            # Initialize DagsHub
            dagshub.init(
                repo_owner=self.repo_owner, repo_name=self.repo_name, mlflow=True
            )
            ...
            logger.warning(
                f"⚠️ DagsHub setup failed, falling back to local MLflow: {e}"
            )
    ...
    def log_model_training(
        ...
        with mlflow.start_run(
            ...
            mlflow.sklearn.log_model(
                model, "model", registered_model_name=f"helios_grid_{model_type}"
            )
    ...
    def log_prediction_batch(
        ...
        with mlflow.start_run(
            run_name=f"prediction_batch_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ):
    ...
    def log_system_metrics(
        self, api_metrics: Dict[str, float], model_metrics: Dict[str, float]
    ):
        """Log system performance metrics"""

        with mlflow.start_run(
            run_name=f"system_metrics_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ):
    ...
    def compare_models(self, experiment_name: str = "helios-grid-production"):
        ...
        try:
            ...
            training_runs = runs[
                runs["tags.mlflow.runName"].str.contains("training", na=False)
            ]
            ...
            for idx, run in best_runs.iterrows():
                ...
                logger.info(
                    f"   {model_type:15} | R²: {test_r2:6} | RMSE: {test_rmse:8} | ID: {run_id[:8]}"
                )
            ...
            try:
                ...
                if models:
                    ...
                    latest_version = client.get_latest_versions(
                        latest_model.name, stages=["Production", "Staging", "None"]
                    )[0]
