...
class GitHubActionsLogReader:
    ...
    def get_workflow_runs(
        self, branch: str = "feature/premium-mobile-ui-cicd", limit: int = 5
    ) -> List[Dict]:
    ...
    def analyze_logs(self, logs: str) -> Dict:
        ...
        for i, line in enumerate(lines):
            ...
            # Extract warnings
                ...
                keyword in line_lower for keyword in ["warning:", "warn:", "deprecated"]
            ):
                analysis["warnings"].append({"line": i + 1, "content": line.strip()})

            # Extract test results
            elif any(
                keyword in line_lower
                for keyword in ["passed", "failed", "skipped", "test"]
            ):
            ...
            # Extract build information
                ...
                keyword in line_lower
                for keyword in ["installing", "building", "compiling"]
            ):
                analysis["build_info"].append({"line": i + 1, "content": line.strip()})

            # Extract security issues
            elif any(
                keyword in line_lower
                for keyword in ["vulnerability", "cve-", "security"]
            ):
    ...
    def _get_context(
        self, lines: List[str], index: int, context_size: int = 2
    ) -> List[str]:
    ...
    def get_latest_run_analysis(
        self, branch: str = "feature/premium-mobile-ui-cicd"
    ) -> str:
        ...
        print(
            f"   Status: {latest_run['status']} | Conclusion: {latest_run.get('conclusion', 'N/A')}"
        )
