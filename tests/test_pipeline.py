...
class TestDataIngestion:
    ...
    def test_azure_storage_setup(
        self, mock_blob_client, mock_credential, sample_config
    ):
...
class TestModelTraining:
    ...
    def sample_config(self):
        return ModelConfig(
            model_type="random_forest", cv_folds=3, n_trials=5, random_state=42
        )
    ...
    def test_predict_endpoint_success(self, client, mock_model_manager):
        ...
        with patch("src.api.main.verify_token", return_value="valid_token"):
            response = client.post(
                "/predict", json=payload, headers={"Authorization": "Bearer test_token"}
            )
...
class TestDriftDetection:
    ...
    def test_numerical_drift_detection(self, reference_data):
        ...
        result = detector._test_numerical_drift(
            "feature1", reference_series, drifted_series
        )
    ...
    def test_categorical_drift_detection(self, reference_data):
        ...
        drifted_series = pd.Series(
            np.random.choice(["A", "B", "C", "D"], 500)
        )  # New category
        reference_series = reference_data["feature2"]

        result = detector._test_categorical_drift(
            "feature2", reference_series, drifted_series
        )
...
if __name__ == "__main__":
    ...
    pytest.main(["--cov=src", "--cov-report=html", "--cov-report=term-missing", "-v"])
