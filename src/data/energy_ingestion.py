    ...
    def __post_init__(self):
        if self.feature_columns is None:
            self.feature_columns = [
                ...
                'is_weekend', 'season', 'temperature_lag',
...
class EnergyConsumptionIngestion:
    ...
    def _load_kaggle_credentials(self) -> Dict[str, str]:
        ...
        try:
            ...
            kaggle_path = Path("C:/Users/sanke/Documents/MLops/Project-Aegis-Finance/_temp/kaggle.json")
    ...
    def _create_time_features(self, df: pd.DataFrame, datetime_col: str) -> pd.DataFrame:
    ...
    def _clean_target_column(self, df: pd.DataFrame, target_col: str) -> pd.DataFrame:
        ...
        if negative_mask.sum() > 0:
            logger.warning(f"Removing {negative_mask.sum()} negative values from target column")
        ...
        if outlier_mask.sum() > 0:
            logger.warning(f"Capping {outlier_mask.sum()} extreme outliers in target column")
    ...
    def _remove_outliers(self, df: pd.DataFrame, target_col: str) -> pd.DataFrame:
        ...
        if outliers_count > 0:
            logger.info(f"Removing {outliers_count} outliers ({outliers_count/len(df)*100:.2f}%)")
            df = df[~outlier_mask]
    ...
    def validate_energy_data(self, df: pd.DataFrame) -> bool:
        ...
        try:
            ...
            if len(df) < self.config.validation_rules['min_records']:
                logger.error(f"Dataset too small: {len(df)} < {self.config.validation_rules['min_records']}")
                return False
    ...
    def process_energy_pipeline(self) -> Dict[str, Any]:
        ...
        try:
            ...
            blob_url = self.upload_to_azure(df, "processed/energy_consumption_processed.csv")
            ...
            result = {
                ...
                "date_range": {
                    "start": str(df.iloc[0]['datetime']) if 'datetime' in df.columns else "unknown",
                    "end": str(df.iloc[-1]['datetime']) if 'datetime' in df.columns else "unknown"
                },
