...
class EnergyFeatureConfig:
    ...
    # Feature selection
    feature_selection_method: str = "mutual_info"  # 'f_regression', 'mutual_info', 'both'
    max_features: int = 50
...
class EnergyFeatureEngineer(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series = None):
        ...
        logger.info(f"Feature engineering fitted with {len(self.feature_names_)} features")
        return self
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.scaler is not None:
            numeric_features = X_features.select_dtypes(include=[np.number]).columns
            if len(numeric_features) > 0:
                X_features[numeric_features] = self.scaler.transform(X_features[numeric_features])
    ...
    def _identify_target_column(self, df: pd.DataFrame) -> Optional[str]:
        ...
        target_candidates = [
            'energy_consumption', 'consumption', 'demand', 'load',
            'mwh', 'kwh', 'power', 'usage', 'Energy_Consumption',
            'PJME_MW', 'AEP_MW', 'COMED_MW', 'DAYTON_MW'
        ]
    ...
    def _create_temporal_features(self, df: pd.DataFrame, datetime_col: str) -> pd.DataFrame:
    ...
    def _create_rolling_features(self, df: pd.DataFrame, target_col: str) -> pd.DataFrame:
        ...
        for window in self.config.rolling_windows:
            ...
            df[f'{target_col}_rolling_volatility_{window}'] = (
                df[f'{target_col}_rolling_std_{window}'] / df[f'{target_col}_rolling_mean_{window}']
            )
        
        logger.info(f"Created rolling features for {len(self.config.rolling_windows)} windows")
        return df
    ...
    def _create_interaction_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'is_business_hour' in df.columns and 'day_of_week' in df.columns:
            df['business_hour_weekday'] = df['is_business_hour'] * (1 - df['is_weekend'])
    ...
    def _create_statistical_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        base_numeric_cols = [col for col in numeric_cols 
                           if not any(keyword in col for keyword in ['lag', 'rolling', 'diff', 'pct_change'])]
...
class EnergyFeaturePipeline:
    ...
    def process_features(self, 
                        ...
                        test_df: pd.DataFrame = None) -> Tuple[pd.DataFrame, pd.DataFrame, pd.Series]:
        ...
        with mlflow.start_run(nested=True, run_name="energy_feature_engineering"):
            ...
            mlflow.log_param("feature_selection_method", self.config.feature_selection_method)
            ...
            logger.info(f"Feature engineering completed: {X_train_processed.shape[1]} features")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Please run energy_ingestion.py first to download and process the dataset")
    except Exception as e:
        print(f"Error: {e}")
        print("Make sure the energy consumption dataset has been downloaded and processed")