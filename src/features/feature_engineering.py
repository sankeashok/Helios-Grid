...
class AdvancedFeatureEngineer(BaseEstimator, TransformerMixin):
    ...
    def _create_numerical_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'GrLivArea' in df.columns and 'BedroomAbvGr' in df.columns:
            df_enhanced['AreaPerBedroom'] = df_enhanced['GrLivArea'] / (df_enhanced['BedroomAbvGr'] + 1)
        
        if 'TotalBsmtSF' in df.columns and 'GrLivArea' in df.columns:
            df_enhanced['TotalSF'] = df_enhanced['TotalBsmtSF'] + df_enhanced['GrLivArea']
    ...
    def _create_categorical_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'Neighborhood' in df.columns and 'HouseStyle' in df.columns:
            df_enhanced['Neighborhood_HouseStyle'] = (
                df_enhanced['Neighborhood'].astype(str) + '_' + 
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series = None):
        ...
        numerical_features = [col for col in X_enhanced.columns 
                            if X_enhanced[col].dtype in ['int64', 'float64']]
        categorical_features = [col for col in X_enhanced.columns 
    ...
    def get_feature_names(self) -> List[str]:
        ...
        if self.feature_selector is not None:
            ...
            all_features = [feat for i, feat in enumerate(all_features) if selected_indices[i]]
...
class FeaturePipeline:
    ...
    def process_features(self, 
                        ...
                        test_df: pd.DataFrame = None) -> Tuple[np.ndarray, np.ndarray, pd.Series]:
        ...
        with mlflow.start_run(nested=True, run_name="feature_engineering"):
            ...
            if test_df is not None:
                X_test = test_df.drop(columns=[self.config.target_column], errors='ignore')
