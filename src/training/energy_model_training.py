...
class EnergyModelTrainer:
    ...
    def train_energy_model(self, X_train: pd.DataFrame, y_train: pd.Series, 
        ...
        with mlflow.start_run(run_name=f"energy_training_{self.config.model_type}"):
            logger.info(f"Starting {self.config.model_type} training for energy consumption")
            ...
            if hasattr(self.best_model, 'fit') and 'early_stopping_rounds' in self.best_model.get_params():
            ...
            if hasattr(self.best_model, 'feature_importances_'):
                importance_dict = dict(zip(X_train.columns, self.best_model.feature_importances_))
    ...
    def _calculate_energy_metrics(self, y_true: pd.Series, y_pred: np.ndarray) -> Dict[str, float]:
    ...
    def register_model(self, model_name: str, description: str = None) -> Optional[str]:
        ...
        if not self.workspace:
            logger.warning("Azure ML workspace not available, skipping model registration")
            return None
        ...
        try:
            ...
            model = Model.register(
                ...
                description=description or f"Energy consumption {self.config.model_type} model",
                tags={
                    'model_type': self.config.model_type,
...
class EnergyEnsembleTrainer:
    ...
    def train_ensemble(self, X_train: pd.DataFrame, y_train: pd.Series,
        ...
        with mlflow.start_run(run_name="energy_ensemble_training"):
            ...
            for i, config in enumerate(self.models_config):
                logger.info(f"Training model {i+1}/{len(self.models_config)}: {config.model_type}")
                ...
                mlflow.log_metric(f'model_{i}_{config.model_type}_cv_rmse', result['cv_rmse'])
            
            # Optimize ensemble weights
            val_predictions = np.column_stack(val_predictions)
            self.ensemble_weights = self._optimize_ensemble_weights(val_predictions, y_val)
            
            # Calculate ensemble metrics
            ensemble_pred = np.average(val_predictions, weights=self.ensemble_weights, axis=1)
    ...
    def _optimize_ensemble_weights(self, predictions: np.ndarray, y_true: pd.Series) -> np.ndarray:
        """Optimize ensemble weights using Optuna"""
        
        def objective(trial):
            weights = [trial.suggest_float(f'weight_{i}', 0, 1) 
        ...
        weights = np.array([study.best_params[f'weight_{i}'] 
                           for i in range(predictions.shape[1])])
        return weights / weights.sum()
    
    def _calculate_ensemble_metrics(self, y_true: pd.Series, y_pred: np.ndarray) -> Dict[str, float]:
...
if __name__ == "__main__":
    try:
        ...
        }).to_csv("data/processed/test_predictions_energy.csv", index=False)
        
        print("Training and prediction completed successfully!")
        
    except FileNotFoundError:
        print("Please run energy_feature_engineering.py first to create processed features")
    except Exception as e:
        print(f"Error: {e}")
