...
class DriftDetector:
    ...
    def _test_feature_drift(self, column: str, reference: pd.Series, current: pd.Series) -> Dict:
    ...
    def _test_numerical_drift(self, column: str, reference: pd.Series, current: pd.Series) -> Dict:
        ...
        mean_shift = abs(curr_mean - ref_stats['mean']) / ref_stats['std'] if ref_stats['std'] > 0 else 0
    ...
    def _test_categorical_drift(self, column: str, reference: pd.Series, current: pd.Series) -> Dict:
        ...
        ref_aligned = pd.Series([ref_dist.get(cat, 0) for cat in all_categories], index=all_categories)
        curr_aligned = pd.Series([curr_dist.get(cat, 0) for cat in all_categories], index=all_categories)
...
class ModelPerformanceMonitor:
    ...
    def evaluate_performance(self, X: pd.DataFrame, y_true: pd.Series) -> Dict[str, Any]:
            ...
            for metric, current_value in current_metrics.items():
                if metric in self.reference_performance:
                    ...
                    if metric in ['rmse', 'mae', 'mape']:  # Lower is better
                        degradation[f'{metric}_degradation'] = (current_value - ref_value) / ref_value
                    else:  # Higher is better (e.g., r2_score)
                        degradation[f'{metric}_degradation'] = (ref_value - current_value) / ref_value
...
class AzureMonitoringPipeline:
    ...
    def run_monitoring_pipeline(self, 
                               ...
                               model=None,
                               y_true: pd.Series = None) -> Dict[str, Any]:
    ...
    def _generate_alerts(self, results: Dict) -> List[Dict]:
        ...
        if results['data_drift'].get('overall_drift', False):
            alerts.append({
                'type': 'data_drift',
                'severity': 'high' if results['data_drift']['drift_score'] > 0.3 else 'medium',
    ...
    def _generate_evidently_report(self, reference_data: pd.DataFrame, 
    ...
    def _log_to_mlflow(self, results: Dict):
        ...
        try:
            with mlflow.start_run(run_name="monitoring_run"):
                ...
                mlflow.log_metric("overall_drift_score", drift_data.get('drift_score', 0))
                mlflow.log_metric("drift_detected", int(drift_data.get('overall_drift', False)))
