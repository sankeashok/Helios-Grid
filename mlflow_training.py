...
class HeliosGridMLflowTrainer:
    ...
    def create_synthetic_dataset(self, n_samples: int = 10000) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        time_effect = 300 * (0.5 + 0.3 * np.sin(hour_normalized - np.pi/2)) + 200 * ((hour >= 18) & (hour <= 22))
        ...
        y = (base_consumption + temp_effect + time_effect + seasonal_effect + 
             weekend_effect + solar_effect + wind_effect + humidity_effect + 
    ...
    def compare_models(self, X: pd.DataFrame, y: pd.Series) -> Dict[str, Any]:
        ...
        best_model_type = max(results.keys(), key=lambda k: results[k]["metrics"]["test_r2"])
...
def main():
    ...
    for model_type, metrics in comparison_results["comparison_summary"].items():
        logger.info(f"   {model_type:20} | R²: {metrics['test_r2']:.4f} | RMSE: {metrics['test_rmse']:.2f}")
