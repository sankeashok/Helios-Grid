...
jobs_url = (
    f"https://api.github.com/repos/sankeashok/Helios-Grid/actions/runs/{run_id}/jobs"
)
...
with open("logs.txt", "w", encoding="utf-8") as f:
    for job in jobs_data["jobs"]:
        icon = (
            ...
            else ("FAIL" if job["conclusion"] == "failure" else str(job["conclusion"]))
        )
        ...
        if job["conclusion"] == "failure":
            for step in job["steps"]:
                if step["conclusion"] == "failure":
                    step_name = (
                        step["name"].encode("ascii", "ignore").decode("ascii").strip()
                    )
